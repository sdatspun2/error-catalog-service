...
An API's [definition](http://spec.openapis.org/oas/v3.0.3#openapi-document) can be used by documentation generation tools to display the API, code generation tools to generate servers and clients in various programming languages, testing tools, and many other use cases.
...
Therefore, documentation generation tools cannot list and describe problem types with API documentation based on just the API definition. Similarly, contract-based testing for the API cannot cover error scenarios comprehensively. Lastly, an API client can not handle error scenarios and write a robust client-side code unless the API developer has put an extra effort in listing error types with the API definition and provided curated and extensive error documentation for the API using other means.

### Sending errors with problem types is good but questions remain
...
Let's take an example error response from RFC 7807 to understand what else needs to be addressed. Note that even if an HTTP API is not using the error schema suggested by RFC 7807, following questions are still relevant.
...
The [API definition](error-catalog-service.json) for the Error Catalog Service itself is annotated with the Error Type extension as defined in [OpenAPIExtensionForErrorType](./OpenAPIExtensionForErrorType.md).

**Note**: Since API described has no service implementation, the API definition uses a fictitious variable called `{ecid}` as error catalog id.
